...
static int ephy_init(void)
{
	...
	ret = phy_drivers_register(&ac300_ephy_driver, 1, THIS_MODULE);
	if (ret)
		goto ac300_ephy_error;

	ret = phy_drivers_register(&sunxi_phy_driver, 1, THIS_MODULE);
	if (ret)
...
ephy_driver_error:
	phy_drivers_unregister(&ac300_ephy_driver, 1);
...
{
	phy_drivers_unregister(&sunxi_phy_driver, 1);
	phy_drivers_unregister(&ac300_ephy_driver, 1);
	platform_driver_unregister(&ac200_ephy_driver);
}
