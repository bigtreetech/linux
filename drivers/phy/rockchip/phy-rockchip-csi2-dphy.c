...
static int csi2_dphy_update_sensor_mbus(struct v4l2_subdev *sd)
{
	...
	if (ret) {
		dev_err(dphy->dev, "%s get_mbus_config fail: %d, pls check it\n",
			sensor_sd->name, ret);
		return ret;
	}
					   ...
					   struct v4l2_async_connection *asd)
{
	...
	dev_info(dphy->dev, "alan: rockchip_csi2_dphy_notifier_bound\n");
...
static int rockchip_csi2_dphy_fwnode_parse(struct csi2_dphy *dphy)
{
	...
	struct v4l2_fwnode_endpoint ovep = {
		.bus_type = V4L2_MBUS_CSI2_DPHY
	};
	struct v4l2_fwnode_endpoint *vep = &ovep;
	struct device *remote_dev = NULL;
	int ret;
	
	dev_info(dphy->dev, "alan: rockchip_csi2_dphy_fwnode_parse\n");
	...
	if (vep->bus_type != V4L2_MBUS_CSI2_DPHY &&
		...
		// v4l2_err(&isp_dev->v4l2_dev, "unsupported bus type\n");
		dev_info(dphy->dev, "alan-1: unsupported bus type\n");
		goto err_parse;
	}
	...
	dev_info(dphy->dev, "alan-1: vep.bus_type: %d\n", vep->bus_type);
	...
	// 	ret = v4l2_fwnode_endpoint_parse(ep, &vep);
	// 	if (ret) {
	// 		dev_info(dphy->dev, "alan: err_parse-0\n");
	// 		goto err_parse;
	// 	}

	// 	/* only add fwnode form port 0 to notifier list */
	// 	if (vep.base.port != 0) {
	// 		dev_info(dphy->dev, "alan: vep.base.port: %d\n", vep.base.port);
	// 		continue;
	// 	}
	...
	// 		dev_info(dphy->dev, "alan: skip device dts status is disabled");
	// 		continue;
	// 	}
	...
	// 			if (!remote_dev || !remote_dev->driver) {
	// 				fwnode_handle_put(remote_ep);
	// 				dev_info(dphy->dev, "alan: heck sensor register state on i2c/spi bus for gki");
	// 				continue;
	// 			}
	// 		}
	// 	}
	...
	// 	if (IS_ERR(s_asd)) {
	// 		ret = PTR_ERR(s_asd);
	// 		dev_info(dphy->dev, "alan: err_parse-1\n");
	// 		goto err_parse;
	// 	}

	// 	config = &s_asd->mbus;

	// 	dev_info(dphy->dev, "alan: vep.bus_type: %d\n", vep.bus_type);
	// 	if (vep.bus_type == V4L2_MBUS_CSI2_DPHY ||
	// 	    vep.bus_type == V4L2_MBUS_CSI2_CPHY) {
	// 		dev_info(dphy->dev, "alan: 1\n");
	// 		config->type = vep.bus_type;
	// 		config->bus.mipi_csi2.flags = vep.bus.mipi_csi2.flags;
	// 		s_asd->lanes = vep.bus.mipi_csi2.num_data_lanes;
	...
	// 		ret = -EINVAL;
	// 		dev_info(dphy->dev, "alan: err_parse-2\n");
	// 		goto err_parse;
	// 	}
	// }
	return 0;
...
static int rockchip_csi2dphy_media_init(struct csi2_dphy *dphy)
{
	...
	dev_info(dphy->dev, "alan: rockchip_csi2dphy_media_init\n");
// alan
...
// alan
	...
	dev_info(dphy->dev, "alan: rockchip_csi2dphy_media_init - ok\n");
